...
public class ZStringUtils {
	...
	public static String unescape(String str){
	   ...
	   final  private static String hex(char ch) {
	    	return Integer.toHexString(ch).toUpperCase();
	   } 
	   final  private static char unhex(String s) {
		  return (char) Integer.parseInt(s, 16);
	   } 
	    
	    public static String escapeJavaScript(String js, boolean escapeUnicode, boolean escapeSingleQuote) {
			if (js == null) {
				return null;
			}
					...
					default : 
						if (escapeUnicode) {
							if (ch > 0xfff) {
								...
								rt.append("\\u00" + hex(ch));
							} else if (ch < 32) {
										if (ch > 0xf) {
											rt.append("\\u00" + hex(ch));
										} else {
											rt.append("\\u000" + hex(ch));
										}
							}else {
								rt.append(ch);
							}
	    ...
	    public static String escapeJava(String js, boolean escapeUnicode, boolean escapeSingleQuote) {
			if (js == null) {
				return null;
			}
			StringBuilder rt = new StringBuilder();
...
//						break;
	    ...
	    public static String unescapeJava(String js) {
	    	return unescapeJavaScript(js);
	    }
	    
	    public static String unescapeJavaScript(String js) {
	    	if (js == null) {
				return null;
			}
			StringBuilder rt = new StringBuilder();
				...
				if (ch == '\\') {
					...
					switch (ch) {
						...
						case 'r' :
							...
							break;
						case 'u' :
							rt.append( unhex(js.substring(i+1, i+5)));
							i += 4;
							break;
						default : 
							rt.append(ch);
	...
	public static String escapeXML(String str, boolean over7F){
		if (str == null){
			return null;
		}
	    return entities.escape(str, over7F);
	}
	
	public static String escapeXML(String str, boolean over7F,  boolean cncd){
		if (str == null){
			return null;
		}
	    return entities.escape(str, over7F, cncd);
	}
	
	public static String convert2UnicodeLiteral(String s){
		char[] ca= s.toCharArray();
		StringBuffer rt = new StringBuffer();
		for (int i = 0; i < ca.length; i++){
			if (ca[i] > 127){
				rt.append("\\u");
				String hex = Integer.toHexString(ca[i]);
				int rem = 4 - hex.length();
				for (int j = 0; j < rem; j++){
					rt.append('0');
				}
				rt.append(hex);
...
class Entities {
    ...
    public static final String[][] APOS_ARRAY = {
...
// <!-- rsaquo is proposed but not yet ISO standardized -->
	...
	public static String convert2ISOLiteral(char ch){
		StringBuffer rt = new StringBuffer();
				rt.append("&#");
				String cstr = Integer.toString(ch);
				for (int j = 0; j < 4 - cstr.length(); j++){
					rt.append('0');
				}
				rt.append(cstr);
		return rt.toString();
     ...
     * @param str
     * @param do7F
     * @param cncd whether converter char which is neither digital nor a.z, but less than 7f
     * @return
     */
    public  String escape(String str, boolean do7F, boolean cncd){
        ...
        for (i = 0; i < str.length(); ++i) {
            ...
            if (entityName == null) {
                if (ch > 0x7F && do7F) {
                    buf.append(convert2ISOLiteral(ch));
                }else if (cncd && !Character.isLetterOrDigit(ch)){
                	 buf.append(convert2ISOLiteral(ch));
                }
                else {
                    buf.append(ch);
     ...
     */
    public String escape(String str, boolean do7F) {
        //todo: rewrite to use a Writer
        StringBuffer buf = new StringBuffer(str.length() * 2);
        int i;
        for (i = 0; i < str.length(); ++i) {
